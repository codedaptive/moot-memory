...
Queue teardown now waits for background workers to exit.
The next corpus can open the same store without racing an old task.
Chunk IDs can resolve to their source drawer IDs from the warm map.
Trainable providers can release large in-memory bases after reindex work.
The BM25 cache uses a generation guard against writes during a rebuild.
