...
`Corpus.init(storage:models:)` now throws when `models` is empty.
`bm25TopKBySource` and `InvertedIndexStore.topK` are now async throwing calls.
This lets storage failures surface to the caller.

In disk-backed mode, `InvertedIndexStore` does not keep term-frequency maps in memory after open.
It also does not keep document-length maps in memory after open.
It reads both from durable tables when it builds the BM25 index.
Remove and delete paths clear cached index data right away.

`dropIngestQueue` now cancels and awaits both background workers.
It releases queue leases only after both workers exit.
`sourceIDs(forChunkIDs:)` maps chunk UUIDs to source drawer IDs.
It reads the warm map and does not scan stored bodies.

Trainable providers now expose `releaseBasis`.
Hosts can free large trained arrays after reindex work.
The next embed reloads the basis from durable storage.

The BM25 cache tracks a mutation generation.
A rebuild publishes only if no write landed during its awaits.
